...
namespace Fizz.Modes
{
    class jungleclear
    {
        public static void JungleClear()
        {
            ...
            {
                if (mob.Health <= Spells.W.GetSpellDamage(mob) && Settings.jungle_w_only && Spells.W.IsReady())
                {
                    Spells.W.Cast(mob); Orbwalker.ResetAutoAttack();
                }
                else if (!Settings.lane_w_only && Spells.W.IsReady()) Spells.W.Cast(); Orbwalker.ResetAutoAttack();
            }
